...
export function projectOrchState(events) {
  ...
  for (const event of list) {
    ...
    if (event.kind === "init") {
      ...
      if (payload.task !== undefined) state.task = payload.task || null;
      if (payload.taskPath !== undefined) state.taskPath = payload.taskPath || null;
      if (payload.target !== undefined) state.target = payload.target || null;
      if (payload.workspace !== undefined) state.workspace = payload.workspace || null;
      if (payload.participants !== undefined) state.participants = Array.isArray(payload.participants) ? payload.participants : [];
      if (payload.budget !== undefined) state.budget = payload.budget || null;
    }
