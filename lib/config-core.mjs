export function defaultConfig() {
  return {
    ...
    claudeCli: null,
    claudeCliSearchTimeoutMs: 5000,
    claudeRunnerTimeoutMs: 60000,
    claudeRequireAvailable: false,
    claudeModel: null,
    claudeMaxTurns: 12,
    claudeMaxBudgetUsd: null,
    claudePermissionMode: "deny",
    asciiConsole: true,
    denySessions: [],
    env: {
...
function validateIntegerRange(configObject, key, min, max) {
  if (configObject[key] === null || configObject[key] === undefined) return;
  const value = Number(configObject[key]);
...
export function validateConfig(configObject) {
  ...
  validateIntegerRange(configObject, "claudeCliSearchTimeoutMs", 100, 60000);
  validateIntegerRange(configObject, "claudeRunnerTimeoutMs", 1000, 600000);
  validateIntegerRange(configObject, "claudeMaxTurns", 1, 50);
  if (configObject.claudeMaxBudgetUsd !== null && configObject.claudeMaxBudgetUsd !== undefined) {
    validateNumberRange(configObject, "claudeMaxBudgetUsd", 0.000001, 1000);
  }
  ...
  if (configObject.claudeModel !== null && configObject.claudeModel !== undefined) {
    if (typeof configObject.claudeModel !== "string") throw new Error("claudeModel must be a string or null");
    if (!configObject.claudeModel.trim()) configObject.claudeModel = null;
  }
  if (typeof configObject.claudeRequireAvailable !== "boolean") throw new Error("claudeRequireAvailable must be boolean");
  if (!["deny", "plan", "default"].includes(String(configObject.claudePermissionMode || ""))) {
    throw new Error("claudePermissionMode must be deny, plan, or default");
  }
