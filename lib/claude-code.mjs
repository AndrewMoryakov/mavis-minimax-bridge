...
function spawnCommandFor(file, args) {
  const shimKind = classifyClaudePath(file);
  const useWindowsShellShim = process.platform === "win32" && (shimKind === "cmd-shim" || shimKind === "bat-shim");
  if (!useWindowsShellShim) return { file, args, shell: false, windowsVerbatimArguments: false };
  return {
    file: process.env.ComSpec || "cmd.exe",
    args: ["/d", "/s", "/c", ["call", quoteWindowsCmdArg(file), ...args.map(quoteWindowsCmdArg)].join(" ")],
    shell: false,
...
function runCommandDefault(file, args, options = {}) {
  return new Promise((resolve) => {
    const command = spawnCommandFor(file, args);
...
async function resultFromPath({ configuredCli, filePath, source, fsModule, runCommand, timeoutMs, probe = true }) {
  ...
  const canSpawn = spawnableKinds.has(kind);
  const version = canSpawn && probe
    ? await runCommand(filePath, ["--version"], { timeoutMs })
    : null;
  const available = canSpawn && (!probe || Boolean(version?.ok));
  const versionWarning = canSpawn && probe && !version?.ok
    ? `Claude CLI exists but failed '--version' probe at ${filePath}: ${redactClaudeText(version?.stderr || version?.stdout || version?.error || "unknown error", 1000)}`
    : null;
  return baseCliResult({
    ...
    path: filePath,
    source,
    probe: { which: null, stat, powershell: null, version },
    warning: versionWarning,
    remediation: available ? null : "Install Claude Code CLI or set claudeCli to a spawnable executable, .cmd, or .bat path.",
  });
}
...
async function resolveFromPathLookup({ configuredCli, command, platform, runCommand, fsModule, timeoutMs }) {
  ...
  for (const candidate of sorted) {
    ...
    if (stat.ok) {
      const result = await resultFromPath({
        configuredCli,
        filePath: resolved,
        source: "path",
        fsModule,
      ...
      result.probe.which = which;
      if (result.available) return { result, which };
    }
  }
...
async function powershellProbe({ configuredCli, runCommand, fsModule, timeoutMs }) {
  ...
  if (commandType === "Application") {
    ...
    if (candidate) {
      const resolved = path.resolve(candidate);
      const result = await resultFromPath({ configuredCli, filePath: resolved, source: "powershell", fsModule, runCommand, timeoutMs });
...
export async function resolveClaudeCli(options = {}) {
  ...
  if (configuredCli) {
    ...
    if (path.isAbsolute(resolved)) {
      return resultFromPath({ configuredCli, filePath: resolved, source: "config", fsModule, runCommand, timeoutMs });
    }
  ...
  const defaults = platform === "win32" ? ["claude.exe", "claude.cmd", "claude"] : [defaultClaudeCli(platform)];
  let lastWhich = null;
...
function spawnClaudeProcess(command, args, options) {
  return new Promise((resolve) => {
    const spawnCommand = spawnCommandFor(command, args);
    const child = options.spawnImpl(spawnCommand.file, spawnCommand.args, {
      cwd: options.cwd,
      env: options.env,
      windowsHide: true,
